...
export default function CommentSection({ firstComment }) {
  ...
  const likeButtonOnclick = async () => {
    ...
    await unlike({ targetId: firstComment._id, targetType: "comment" });
        ...
        <div className="flex items-start space-x-3">
          <div className="w-10 h-10 bg-linear-to-br from-orange-500 to-red-500 rounded-full flex items-center justify-center shrink-0">
            <span className="text-white font-semibold text-sm">
              {firstComment?.author.firstName.slice(0, 1).toUpperCase()}
            </span>
          </div>
          <div className="flex-1">
            <div className="bg-gray-50 rounded-lg p-3 mb-2 z-10">
              <h4 className="text-sm font-semibold text-gray-900 mb-1">
                {`${firstComment?.author.firstName} ${firstComment?.author.lastName}`}
              </h4>
            ...
            {/* Nested Reply Input */}
            {nestedReply && (
              <NestedReplyInput setNestedReply={setNestedReply} />
            )}
          </div>
        </div>
      </div>
